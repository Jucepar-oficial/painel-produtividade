...
const FOTOS={"ADELE STIVAL CHENU SCHNEIDER":"adele-stival-chenu-schneider.webp","ANADEJE MELISSA ALEXANDRE STRECHAR":"anadeje-melissa-alexandre-strechar.webp","ALEXANDRE FAYZANO":"alexandre_fayzano.png","BEATRIZ SZPAK":"beatriz-szpak.webp","BIANCA DOMAKOSKI":"bianca-domakoski.png","CELSO MACHADO":"celso-machado.webp","CLAUDIOMIRO SANTOS RODRIGUES":"claudiomiro-santos-rodrigues.webp","ERCÍLIO SANTINONI":"ercilio-santinoni.webp","FABIANA KONIG JUNKES":"fabiana-konig-junkes.webp","FRANCISCO MISURELLI FERRO":"francisco-misurelli-ferro.webp","GIOVANI CÁSSIO PIOVEZAN":"giovani-cassio-piovezan.png","IVO ERICSSON CAMARGO DE LIMA":"ivo-ribeiro-camargo-de-lima.webp","JAIR LEITE":"jair-leite.webp","JOÃO PAULO ATILIO GODRI":"joao-paulo-atilio-godri.webp","JOSÉ LUIZ VARGAS BUENO":"jose-luiz-vargas-bueno.webp","JUÇARA MARQUES NEGREIROS":"jucara-marques-negreiros.webp","MARIA AUGUSTA PISANI GEARA":"maria-augusta-pisani-geara.webp","SAMARA CRISTINA DOS SANTOS DE MEIRA":"samara-cristina-dos-santos-de-meira.png","SÉRGIO PEREIRA LOBO":"sergio-pereira-lobo.webp","CASSIANA MARIA MEDEIROS FRAZÃO MELEK":"cassiana-maria-medeiros-frazao-melek.png","JOSÉ GEORGEVAN GOMES DE ARAÚJO":"jose-georgevan-gomes-de-araujo.png","NAIM AKEL NETO":"naim-akel-neto.png"};
...
const rotuloPeriodo=p=>`${MESES[Number(p.slice(5))-1]} de ${p.slice(0,4)}`;
function avatar(nome){const f=FOTOS[nome];const ini=nome.split(/\s+/).filter(Boolean);return `<div class="avatar">${f?`<img src="imagens/fotos-vogais/${f}" alt="Foto de ${esc(nome)}">`:`${esc((ini[0]?.[0]||'')+(ini.at(-1)?.[0]||''))}`}</div>`}
async function iniciar(){try{const manifest=await fetch('dados/manifest.json').then(r=>{if(!r.ok)throw Error('manifest');return r.json()});calendario=await fetch('dados/calendario.json').then(r=>r.json());const pacotes=await Promise.all(manifest.arquivos.map(a=>fetch(`dados/${a}`).then(r=>{if(!r.ok)throw Error(a);return r.json()})));registros=pacotes.flatMap(p=>p.registros);periodos=[...new Set(registros.map(r=>r.periodo))].sort();const mapa=new Map;registros.forEach(r=>mapa.set(r.vogal,{nome:r.vogal,entidade:r.entidade,sigla:r.siglaEntidade}));vogais=[...mapa.values()].sort((a,b)=>a.nome.localeCompare(b.nome,'pt-BR'));$('atualizacao').textContent=new Date(`${manifest.ultimaAtualizacao}T12:00:00`).toLocaleDateString('pt-BR');$('status-dados').textContent=`${nf.format(soma(registros))} análises disponíveis`;anoSelecionado=periodos.at(-1).slice(0,4);montarExercicios();montarFiltros();renderizar()}catch(e){$('status-dados').textContent='Não foi possível carregar os dados';console.error(e)}}
function opcoes(id,valores,rotulo){const el=$(id);el.innerHTML=(rotulo?`<option value="">${rotulo}</option>`:'')+valores.map(([v,t])=>`<option value="${esc(v)}">${esc(t)}</option>`).join('')}
function periodosDoAno(){return periodos.filter(p=>p.startsWith(anoSelecionado))}
function montarExercicios(){const anos=[...new Set(periodos.map(p=>p.slice(0,4)))];$('exercicios').innerHTML=anos.map(ano=>`<button type="button" class="${ano===anoSelecionado?'ativo':''}" data-ano="${ano}" aria-pressed="${ano===anoSelecionado}">${ano}</button>`).join('');$('exercicios').querySelectorAll('button').forEach(botao=>botao.onclick=()=>{anoSelecionado=botao.dataset.ano;$('exercicios').querySelectorAll('button').forEach(b=>{const ativo=b.dataset.ano===anoSelecionado;b.classList.toggle('ativo',ativo);b.setAttribute('aria-pressed',ativo)});atualizarPeriodos();if([...$('ano-resumo').options].some(o=>o.value===anoSelecionado))$('ano-resumo').value=anoSelecionado;renderizar()})}
function atualizarPeriodos(){const lista=periodosDoAno();opcoes('inicio',lista.map(p=>[p,rotuloPeriodo(p)]));opcoes('fim',lista.map(p=>[p,rotuloPeriodo(p)]));$('inicio').value=lista[0];$('fim').value=lista.at(-1)}
function montarFiltros(){atualizarPeriodos();opcoes('entidade',[...new Map(vogais.map(v=>[v.entidade,v.sigla])).entries()].sort((a,b)=>a[1].localeCompare(b[1],'pt-BR')),'Todas as entidades');opcoes('vogal',vogais.map(v=>[v.nome,v.nome]),'Todos os vogais');opcoes('tipo',[...new Set(registros.map(r=>r.tipoAnalise))].sort().map(x=>[x,x]),'Todos os tipos');opcoes('decisao',[...new Set(registros.map(r=>r.decisao))].sort().map(x=>[x,x==='Exigencia'?'Exigência':x]),'Todas as decisões');opcoes('ano-resumo',[...new Set(periodos.map(p=>p.slice(0,4)))].map(x=>[x,x]));$('ano-resumo').value=anoSelecionado;['inicio','fim','entidade','vogal','tipo','decisao','ano-resumo'].forEach(id=>$(id).addEventListener('change',()=>{if(id==='entidade')atualizarVogais();renderizar()}));$('limpar').onclick=()=>{['entidade','vogal','tipo','decisao'].forEach(x=>$(x).value='');atualizarPeriodos();renderizar()};$('baixar').onclick=baixarCsv}
